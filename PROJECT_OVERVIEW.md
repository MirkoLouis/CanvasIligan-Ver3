...
*   **Application Server and API Gateway:** A **Node.js** server running the **Express.js** framework.
    *   **Rationale:** Node.js was selected for its asynchronous, non-blocking I/O model, which is highly performant for I/O-bound operations such as handling numerous concurrent HTTP requests and proxying them to other services. Express.js provides a minimal, unopinionated layer for building the RESTful API endpoints that serve as the gateway to the system's other components.
